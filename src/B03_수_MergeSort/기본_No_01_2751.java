...
//        BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
    ...
    public static void mergeSort(int start, int end) {    // 숫자열을 가져와서 1개 단위로 쪼개는 행위
        int mid;

        if(start<end) {
            mid = (start+end)/2;
            mergeSort(start,mid);    // 총 8개의 원소를 갖는 배열이 있다면, 맨 앞의 0번째 1번째 인덱스가 하나로 쪼개질 때까지 쪼갠다.
            mergeSort(mid+1, end);

            merge(start, mid, end);    // 다 쪼개면, 하나로 합친다.
        }
    }
